...
def check_output_path(path: str, ext: str, input_path: str = None):
    """
    Checks if the output path exists and creates it if it doesn't.
    If the output path is a directory, a default filename is returned.
    If the output path is a file, it is returned as is, unless the 
    file extension is different from the expected one.
    ...
    Returns:
        str: The output path, either as is or with a modified filename.
    """
    if os.path.isdir(path):
        os.makedirs(path, exist_ok=True)
        if input_path is None:
            return os.path.join(path, f"model.{ext}")
        filename, _ = os.path.splitext(os.path.basename(input_path))
        return os.path.join(path, f"{filename}.{ext}")
    else:
        _, ext_ = os.path.splitext(path)
        if ext_ != ext:
            LOGGER.warning(f"Output file extension {ext_} is different from {ext}")
        return path
